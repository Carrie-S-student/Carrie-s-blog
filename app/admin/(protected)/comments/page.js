...
/**
 * 递归渲染评论项（后台管理，支持任意深度嵌套）。
 */
function AdminCommentItem({ comment, depth }) {
        ...
        {comment.children && comment.children.length > 0 && (
          <div className="mt-3 ml-4 space-y-2">
            {comment.children.map((child) => (
              <AdminCommentItem key={child.id} comment={child} depth={depth + 1} />
            ))}
          </div>
        )}
      </div>
    ...
    <div>
      ...
      <div className="mt-6 space-y-4">
        ...
        {comments.map((comment) => (
          <AdminCommentItem key={comment.id} comment={comment} depth={0} />
        ))}
      </div>
    </div>
  );

...
/**
 * 保护 LaTeX 数学公式，避免被 marked 转义或误处理。
 * 返回 { processed, mathPlaceholders }，后续由 postprocessMath 恢复为 KaTeX HTML。
 *
 * 支持：
 *   $E=mc^2$        → 行内公式
 *   $$\int_0^1$$    → 块级公式（独立一行）
...
function preprocessMath(md) {
  ...
  // ---- 先保护块级公式（$$...$$），避免被行内正则误匹配 ----
  let result = md.replace(/\$\$([\s\S]*?)\$\$/g, (_match, content) => {
    const idx = mathPlaceholders.length;
    mathPlaceholders.push({ type: "block", content: content.trim() });
    return `\x00MATHBLOCK_${idx}\x00`;
  });

  // ---- 再保护行内公式（$...$），排除 $$ 残留 ----
  result = result.replace(/\$([^$\n]+?)\$/g, (_match, content) => {
    const idx = mathPlaceholders.length;
    mathPlaceholders.push({ type: "inline", content: content.trim() });
    return `\x00MATHINLINE_${idx}\x00`;
  });

  return { processed: result, mathPlaceholders };
}

/**
 * 将数学占位符恢复为 KaTeX 兼容 HTML。
 * 客户端 PostContentRenderer 会用 katex.render() 进一步渲染为 SVG/HTML。
 */
function postprocessMath(html, mathPlaceholders) {
  if (!mathPlaceholders || mathPlaceholders.length === 0) return html;

  return html.replace(/\x00MATH(BLOCK|INLINE)_(\d+)\x00/g, (_m, type, idx) => {
    const entry = mathPlaceholders[+idx];
    if (!entry) return _m;
    if (type === "BLOCK") {
      return `<div class="math math-block">${entry.content}</div>`;
    }
    return `<span class="math math-inline">${entry.content}</span>`;
  });
}
...
/**
 * 解析 Markdown 为 HTML：
 * - 预处理 Obsidian 语法（链接、图片、注释、嵌入）
 * - 保护数学公式（$...$ / $$...$$）→ 占位符
 * - 预处理高亮 ==text== → <mark>（占位符保护代码块）
 * - marked GFM 自动处理 表格 / 任务列表 / 删除线 / 脚注
 * - 后处理 Callout 块
 * - 恢复数学公式为 KaTeX 兼容 HTML
 * - 收集需手动上传的本地图片引用
 *
 * ⚠️ Mermaid 图表（```mermaid）不在服务端渲染 ——
 *    由客户端 PostContentRenderer 在浏览器中渲染。
 *
 * 返回 { html, imagesToUpload }
 */
export async function parseMarkdownToHtml(content) {
  // Step 1: 预处理 Obsidian 特有语法
  let processed = preprocessObsidianMarkdown(content);

  // Step 2: 保护数学公式（KaTeX），在代码块保护之前
  const { processed: mathProtected, mathPlaceholders } = preprocessMath(processed);

  // Step 3: 预处理高亮（==text== → <mark>），采用占位符保护代码块
  processed = preprocessHighlight(mathProtected);

  // Step 4: marked 解析（GFM：表格 + 任务列表 + 删除线 + 脚注）
  const rawHtml = await marked.parse(processed, {
    gfm: true,
    breaks: false,
  });

  // Step 5: 后处理 Obsidian Callout 块
  let html = postprocessCallouts(rawHtml);

  // Step 6: 恢复数学公式为 KaTeX 兼容 HTML
  html = postprocessMath(html, mathPlaceholders);

  // Step 7: 收集需要手动上传的本地图片引用
